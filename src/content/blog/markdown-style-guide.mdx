...
Use `**` around text for bold, `_` for italics, and backticks for inline code.
...
For a quote from someone else, include their name and a source. Here, `<br />` puts the name on a new line. `[^1]` links to a footnote, with the source defined on the line starting with `[^1]:`.
